...
from app.services.interfaces import IWorkUnitService, ICycleSubjectService, IPerformanceService
...
class SubjectDetailsController(QObject):
    def __init__(self, view: SubjectDetailsView, cycle_subject: CycleSubject,
                 v20_diagnostics: dict, work_unit_service: IWorkUnitService,
                 cycle_subject_service: ICycleSubjectService, performance_service: IPerformanceService):
        super().__init__(view)
        self._view = view
    ...
    def load_data(self):
        ...
        if total_questions < 10:
            log.debug(f"Subject '{self.cycle_subject.name}' has < 10 questions. Showing 'Getting Started' view.")
            self._view.set_view_mode('getting_started')
        else:
            log.debug(f"Subject '{self.cycle_subject.name}' has sufficient data. Showing 'Performance Hub' view.")
    ...
    def _load_work_units(self):
        ...
        work_units = self.work_unit_service.get_work_units_for_subject(self.cycle_subject.subject_id)
    ...
    def _on_prioritize(self):
        ...
        if current_difficulty >= 5:
            QMessageBox.information(self._view, "Max Priority", "This subject is already at maximum priority (Difficulty 5/5).")
            return

        new_difficulty = current_difficulty + 1
        self.cycle_subject_service.update_cycle_subject_difficulty(self.cycle_subject.id, new_difficulty)
        self.cycle_subject.difficulty_weight = new_difficulty

        QMessageBox.information(
            self._view, "Priority Increased",
