...
from app.models.session import QuestionPerformance, StudySession, SubjectPerformance
from app.models.subject import CycleSubject, Subject, Topic, WorkUnit
from app.models.user import HumanFactor, User
...
class IStudyQueueService(Protocol):
    ...
    def get_next_in_queue(
        self, cycle_id: int, position: int
...
class IPerformanceService(Protocol):
    ...
    def get_performance_over_time(
        self, user_id: int, subject_id: int | None = None
    ) -> List[dict]:
        """
    ...
    def get_topic_performance(
        self, user_id: int, subject_id: int, days_ago: int | None = None
    ) -> List[dict]:
        """
    ...
    def get_study_session_summary(
        self, user_id: int, days_ago: int | None = None
    ) -> dict[int, int]:
        """
    ...
    def get_subject_summary_for_analytics(
        self, user_id: int, cycle_id: int, days_ago: int | None = None
    ) -> List[dict]:
        """
...
class IExamService(Protocol):
    ...
    def create(
        ...
        has_edital: int = 0,
        predicted_exam_date: Optional[str] = None,
    ) -> int: ...
...
class IUserService(Protocol):
    ...
    def save_human_factor(
        self, user_id: int, date: str, energy: str, stress: str
    ) -> int: ...

    def get_human_factor_history(
        self, user_id: int, limit: int = 20
    ) -> List[HumanFactor]: ...

    def update_user(
        self, user_id: int, name: str, study_level: str, theme: str
    ) -> Optional[User]: ...
...
class IAnalyticsService(Protocol):
    ...
    def get_daily_performance(
        self, cycle_id: int, days_ago: Optional[int] = None
