...
class SqliteWorkUnitService(BaseService, IWorkUnitService):
    ...
    def add_work_unit(self, subject_id: int, unit_data: dict) -> Optional[WorkUnit]:
        try:
            ...
            self._execute_query("UPDATE work_units SET unit_id = ? WHERE id = ?", (unit_id_str, new_id))
            new_unit_row = self._execute_query("SELECT * FROM work_units WHERE id = ?", (new_id,)).fetchone()
    ...
    def add_topics_bulk(self, subject_id: int, topic_names: List[str]):
        ...
        self._execute_query("INSERT OR IGNORE INTO topics (subject_id, name) VALUES (?, ?)", data_to_insert)